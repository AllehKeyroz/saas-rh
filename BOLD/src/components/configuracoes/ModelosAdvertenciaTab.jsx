...
function ModeloForm({ open, onClose, modelo, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    if (modelo?.id) {
      await base44.entities.ModeloAdvertencia.update(modelo.id, form);
    } else {
      await base44.entities.ModeloAdvertencia.create(form);
    }
    setSaving(false);
    onSaved();
    onClose();
...
export default function ModelosAdvertenciaTab() {
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.ModeloAdvertencia.delete(id);
