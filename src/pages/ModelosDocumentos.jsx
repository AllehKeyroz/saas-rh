...
export default function ModelosDocumentos() {
  ...
  const { data: modelos = [], isLoading: loadingModelos } = useQuery({
    ...
    queryFn: () => client.entities.ModeloDocumento.list('-created_date'),
  });

  const { data: finalidades = [], isLoading: loadingFinalidades } = useQuery({
    queryKey: ['finalidades-documentos'],
    queryFn: () => client.entities.FinalidadeDocumento.list('-created_date'),
  });
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Crie modelos de contratos, formulários e documentos com variáveis dinâmicas e integração GovBR</p>
      </div>
