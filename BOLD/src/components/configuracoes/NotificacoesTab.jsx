...
function NotificacaoForm({ open, onClose, notificacao, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    if (notificacao) {
      await base44.entities.ConfiguracaoNotificacao.update(notificacao.id, form);
    } else {
      await base44.entities.ConfiguracaoNotificacao.create(form);
    }
    setSaving(false);
    onSaved();
    onClose();
...
export default function NotificacoesTab() {
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.ConfiguracaoNotificacao.delete(id);
    queryClient.invalidateQueries({ queryKey: ['notificacoes'] });
  };

  const handleToggle = async (notif) => {
    await base44.entities.ConfiguracaoNotificacao.update(notif.id, { ativo: !notif.ativo });
    queryClient.invalidateQueries({ queryKey: ['notificacoes'] });
  };
