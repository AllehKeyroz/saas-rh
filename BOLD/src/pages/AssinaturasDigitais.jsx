...
export default function AssinaturasDigitais() {
  ...
  const { data: assinaturas = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.AssinaturaDigital.list('-data_envio'),
  });

  const { data: funcionarios = [] } = useQuery({
    queryKey: ['funcionarios'],
    queryFn: () => base44.entities.Funcionarios.filter({ ativo: true }),
  });
  ...
  const handleCancelar = async (doc) => {
    ...
    await base44.entities.AssinaturaDigital.update(doc.id, { status: 'cancelado' });
  ...
  const handleReenviar = async (doc) => {
    await base44.entities.AssinaturaDigital.update(doc.id, { data_envio: new Date().toISOString() });
    await registrarAuditoria({
