...
async function executarBackup(tipo, queryClient, setGerandoTipo) {
  ...
  const registro = await base44.entities.BackupRegistro.create({
  ...
  try {
    if (tipo === 'documentos' || tipo === 'geral') {
      ...
      const documentos = await base44.entities.DocumentoFuncionario.list();
      ...
      const fichas = await base44.entities.FichaFinanceira.list();
    ...
    if (tipo === 'geral') {
      ...
      const [funcionarios, lancamentos, comissoes, assinaturas, advertencias, fechamentos] = await Promise.all([
        ...
        base44.entities.FichaFinanceira.list(),
        base44.entities.ComissoesGorjetas.list(),
        base44.entities.AssinaturaDigital.list(),
        base44.entities.Advertencias.list(),
    ...
    await base44.entities.BackupRegistro.update(registro.id, {
      file_url,
      status: 'concluido',
    ...
    await base44.entities.BackupRegistro.update(registro.id, { status: 'erro' });
    toast.error(`Erro ao gerar backup: ${error.message}`);
  }

  // Limpar backups expirados
  const todos = await base44.entities.BackupRegistro.list();
  for (const r of todos) {
    if (r.expira_em && new Date(r.expira_em) < new Date()) {
      await base44.entities.BackupRegistro.delete(r.id);
    }
  }
...
export default function BackupsTab() {
  ...
  const { data: backups = [], isLoading } = useQuery({
    queryKey: ['backups'],
    queryFn: () => base44.entities.BackupRegistro.list('-data_geracao'),
  });

  const { data: logsBackup = [] } = useQuery({
    queryKey: ['logsBackup'],
    queryFn: () => base44.entities.LogAuditoria.filter({ modulo: 'backup' }, '-created_date', 20),
  });

  const handleDelete = async (id) => {
    if (!confirm('Excluir este registro de backup?')) return;
    await base44.entities.BackupRegistro.delete(id);
    queryClient.invalidateQueries({ queryKey: ['backups'] });
  };
