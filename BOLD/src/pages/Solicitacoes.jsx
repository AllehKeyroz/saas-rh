...
function ResponderModal({ solicitacao, onClose, onSaved, meUser }) {
  ...
  const handleSalvar = async () => {
    ...
    await base44.entities.SolicitacoesFuncionario.update(solicitacao.id, {
      status,
    ...
    await base44.entities.MensagensRH.create({
...
function ResponderLoteModal({ selecionadas, solicitacoes, onClose, onSaved, meUser }) {
  ...
  const handleSalvar = async () => {
    ...
    await Promise.all(items.map(async (s) => {
      ...
      await base44.entities.SolicitacoesFuncionario.update(s.id, {
        status,
        resposta_rh: resposta,
        respondido_por: meUser?.email || '',
        data_resposta: agora,
...
export default function Solicitacoes() {
  ...
  const { data: solicitacoes = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.SolicitacoesFuncionario.list('-created_date', 200),
  });
  ...
  useEffect(() => {
    const unsubscribe = base44.entities.SolicitacoesFuncionario.subscribe((event) => {
