...
function SolicitacaoModal({ tipo, funcionario, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await base44.entities.SolicitacoesFuncionario.create({
        ...form,
...
export default function MinhasSolicitacoes({ funcionario }) {
  ...
  const { data: solicitacoes = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.SolicitacoesFuncionario.filter({ funcionario_id: funcionario?.id }),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Botões rápidos */}
      {isAtiva('solicitacoes_botoes_rapidos') !== false && (
        <div>
