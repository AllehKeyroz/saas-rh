...
import { Loader2, Send, Upload, FileText, Shield, LayoutTemplate } from 'lucide-react';
...
export default function EnviarDocumentoDialog({ open, onClose, funcionarios, onSucesso }) {
  ...
  const { data: modelos = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ModeloDocumento.filter({ ativo: true }),
    enabled: open,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const novaAssinatura = await base44.entities.AssinaturaDigital.create({
