...
export default function AlertaLimiteVale({ totalValesMes, limite, onVerDetalhes, salarioBase, ajudaCusto }) {
  ...
  const percentual = (totalValesMes / limiteEfetivo) * 100;
  ...
  const config = percentual >= 100
    ? {
        ...
        msg: `Limite de vales atingido! Você utilizou ${formatCurrency(totalValesMes)} de ${formatCurrency(limiteEfetivo)}.`,
      }
    : percentual >= 80
    ? {
        ...
        msg: `Atenção: ${percentual.toFixed(0)}% do limite utilizado (${formatCurrency(totalValesMes)} de ${formatCurrency(limiteEfetivo)}).`,
      }
    : {
        bg: 'bg-blue-50 border-blue-200',
        icon: <Info className="w-5 h-5 shrink-0 mt-0.5 text-blue-500" />,
        textColor: 'text-blue-700',
        msg: `${percentual.toFixed(0)}% do limite de vales utilizado (${formatCurrency(totalValesMes)} de ${formatCurrency(limiteEfetivo)}).`,
      };
