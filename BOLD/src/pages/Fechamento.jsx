...
export default function Fechamento() {
  ...
  const { data: lancamentos = [], isLoading: ll } = useQuery({
    ...
    queryFn: () => base44.entities.FichaFinanceira.list('-created_date', 1000),
  });

  const { data: fechamentos = [], isLoading: lfech } = useQuery({
    queryKey: ['fechamentos'],
    queryFn: () => base44.entities.FechamentoMensal.list(),
  });

  const { data: comissoesFuncionarios = [] } = useQuery({
    queryKey: ['comissoes_funcionarios'],
    queryFn: () => base44.entities.ComissaoPorFuncionario.list('-created_date', 2000),
  });
  ...
  const processarFuncionario = async (func) => {
    ...
    if (jafechado && reprocessMode && isAdmin) {
      await base44.entities.FechamentoMensal.update(jafechado.id, {
      ...
      await base44.entities.FechamentoMensal.create({
    ...
    for (const l of funcLanc) {
      await base44.entities.FichaFinanceira.update(l.id, { consolidado: true });
    }
  };
  ...
  const processarTodos = async () => {
    ...
    // Atualiza limites de vales após processar o fechamento
    try {
      await base44.functions.invoke('calcularLimiteValeMensal', {});
      queryClient.invalidateQueries({ queryKey: ['funcionarios'] });
      toast.success(`Fechamento ${reprocessMode ? 'reprocessado' : 'processado'} com sucesso! Limites de vale atualizados.`);
    } catch {
      toast.success(`Fechamento ${reprocessMode ? 'reprocessado' : 'processado'} com sucesso!`);
      toast.warning('Não foi possível atualizar os limites de vale automaticamente.');
    }
    setProcessing(false);
  ...
  const reabrirFuncionario = async (func) => {
    ...
    await base44.entities.FechamentoMensal.delete(fechado.id);
    // Unconsolidate lancamentos of this month for this employee
    const funcLanc = lancMes.filter(l => l.funcionario_id === func.id && l.consolidado);
    for (const l of funcLanc) {
      await base44.entities.FichaFinanceira.update(l.id, { consolidado: false });
    }
    queryClient.invalidateQueries({ queryKey: ['fechamentos'] });
    queryClient.invalidateQueries({ queryKey: ['lancamentos'] });
    setProcessing(false);
  ...
  const reabrirTodos = async () => {
    ...
    for (const fech of fechamentosMes) {
      await base44.entities.FechamentoMensal.delete(fech.id);
