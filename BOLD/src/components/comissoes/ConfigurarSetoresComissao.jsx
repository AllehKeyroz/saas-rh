  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      for (const s of setoresDB) {
        if (!idsAtuais.includes(s.id)) await base44.entities.SetoresComissao.delete(s.id);
      }
      // Criar ou atualizar
      for (let i = 0; i < lista.length; i++) {
        ...
        if (s.id) await base44.entities.SetoresComissao.update(s.id, data);
        else await base44.entities.SetoresComissao.create(data);
      }
